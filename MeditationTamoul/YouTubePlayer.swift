...
public func videoIDFromYouTubeURL(videoURL: NSURL) -> String? {
    ...
    if let videoId = videoURL.queryStringComponents()["v"] as? String{
        return videoId
    }
    if let host = videoURL.host, pathComponents = videoURL.pathComponents where pathComponents.count > 1 && host.hasSuffix("youtube.com") {
