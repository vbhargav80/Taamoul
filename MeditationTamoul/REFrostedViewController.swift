...
private let kDLHamburguerMenuSpan: CGFloat = UIScreen.mainScreen().bounds.size.width/6.5
 ...
 */
class REFrostedViewController: UIViewController {
    ...
    // structure & hierarchy
    var containerViewController: REFrostedContainerViewController!
    ...
    internal func setupHamburguerViewController() {
        ...
        containerViewController = REFrostedContainerViewController()
        containerViewController.hamburguerViewController = self
        
        // initialize gesture recognizer
        gestureRecognizer = UIPanGestureRecognizer(target: containerViewController!, action: #selector(REFrostedViewController.panGestureRecognized(_:)))
...
extension UIViewController {
    ...
    func findHamburguerViewController() -> REFrostedViewController? {
        var vc = self.parentViewController
        while vc != nil {
            if let dlhvc = vc as? REFrostedViewController { return dlhvc }
...
@objc protocol DLHamburguerViewControllerDelegate {
    optional func hamburguerViewController(hamburguerViewController: REFrostedViewController, didPerformPanGesture gestureRecognizer: UIPanGestureRecognizer)
    optional func hamburguerViewController(hamburguerViewController: REFrostedViewController, willShowMenuViewController menuViewController: UIViewController)
    optional func hamburguerViewController(hamburguerViewController: REFrostedViewController, didShowMenuViewController menuViewController: UIViewController)
    optional func hamburguerViewController(hamburguerViewController: REFrostedViewController, willHideMenuViewController menuViewController: UIViewController)
    optional func hamburguerViewController(hamburguerViewController: REFrostedViewController, didHideMenuViewController menuViewController: UIViewController)
    optional func hamburguerViewController(hamburguerViewController: REFrostedViewController, willTransitionToSize size: CGSize, withTransitionCoordinator coordinator: UIViewControllerTransitionCoordinator)
    // Support for legacy iOS 7 rotation.
    optional func hamburguerViewController(hamburguerViewController: REFrostedViewController, willAnimateRotationToInterfaceOrientation toInterfaceOrientation: UIInterfaceOrientation, duration: NSTimeInterval)
}
